...
> **Addendum (2026-08-21):** the Stitch-generated screens ("Lumina Finance")
> were produced in dark mode, and the project decided to implement the
> design-system foundation with **dark as the current default**, with the
> light palette fully wired (not a stub) via `[data-theme="light"]` so it
> can become default later without rework. Both palettes live as CSS custom
> properties in `src/styles/index.css`. See `memory.md` §23 for the decision
> record.
...
> **Addendum (2026-08-21):** implemented in `src/styles/index.css` as CSS
> custom properties mapped into Tailwind v4 via `@theme inline`, keeping a
> single semantic name (`bg-surface`, `text-on-primary`, `rounded-lg`,
> `text-headline-lg`, `font-label`, …) usable from every component
> regardless of which theme is active. The reusable component library
> living in `src/components/` (Button, Badge, StatusBadge, PriorityBadge,
> Card, Input, Select, Modal, Table, EmptyState, LoadingState, ErrorState)
> consumes only these tokens — no component should introduce a raw hex
> colour, px value, or one-off Tailwind colour class (rules.md §39).
